...
def score_spec_driven_development() -> tuple[float, list[str]]:
    ...
    if ratio < 0.50:
        ...
        return 0.0, notes
    if ratio >= 0.90:
        notes.append(f"regen pass rate {ratio:.0%} â‰¥ 90%; full credit")
...
def score_verification_and_automated_testing() -> tuple[float, list[str]]:
    """Score breakdown matches the v1.4 rubric: 3 + 2 + 2 + 5 = 12 pts.
      ...
      * 2 pts for coverage â‰¥ 70% on business logic
      * 5 pts for user story test pass rate (â‰¥90% = full 5; proportional below)
    """
    ...
    if cov_pct >= 70:
        score += 2
        notes.append(f"coverage {cov_pct:.0f}% â‰¥ 70%: +2")
    else:
        notes.append(f"coverage {cov_pct:.0f}% < 70%: +0")

    # 5 pts for user story pass rate â‰¥90% (proportional below)
...
def score_stress_and_robustness() -> tuple[float, list[str]]:
    ...
    if lt_status.exists() and lt_status.read_text().strip().lower() == "ok":
        score += 4
        notes.append("load test â‰¥10 req/s, <5% errors: +4")
    else:
...
def score_team_contributions() -> tuple[float, list[str]]:
    ...
    #   line 1: percentage match within Â±15 points
...
def main() -> int:
    ...
    (GRADING / "score.json").write_text(encoding='utf-8', data=json.dumps(payload, indent=2))
    ...
    (GRADING / "score.md").write_text(encoding='utf-8', data="\n".join(lines) + "\n")
